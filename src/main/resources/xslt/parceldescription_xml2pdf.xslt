...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:extract="http://geo.so.ch/schema/AGI/LandRegisterParcelDescription/1.0/Extract" exclude-result-prefixes="extract" version="1.0">
  ...
  <xsl:template match="extract:RealEstate">
    <fo:flow flow-name="xsl-region-body">
      ...
      <fo:block-container wrap-option="wrap" hyphenate="false" hyphenation-character="-" font-weight="400" font-size="10pt" font-family="Frutiger" background-color="#FFFFFF">
        <fo:table table-layout="fixed" width="100%" margin-top="8mm">
          ...
          <fo:table-body>
            ...
            <xsl:for-each select="extract:Owner">
              <xsl:sort select="extract:Name"/>
              <fo:table-row border-width="0pt" border-style="solid">
                <fo:table-cell padding-top="1mm" border-style="solid" border-width="0pt">
                  <fo:block>
                    <xsl:value-of select="extract:Name"/><xsl:text> </xsl:text><xsl:value-of select="extract:Vorname"/><xsl:text>, </xsl:text><xsl:value-of select="extract:PostalAddress/extract:Street"/><xsl:text> </xsl:text><xsl:value-of select="extract:PostalAddress/extract:Number"/><xsl:text>, </xsl:text><xsl:value-of select="extract:PostalAddress/extract:PostalCode"/><xsl:text> </xsl:text><xsl:value-of select="extract:PostalAddress/extract:City"/>
                  </fo:block>
                </fo:table-cell>
              </fo:table-row>
            </xsl:for-each>
      ...
      <fo:block-container wrap-option="wrap" hyphenate="false" hyphenation-character="-" font-weight="400" font-size="10pt" font-family="Frutiger" background-color="#FFFFFF">
        <fo:table table-layout="fixed" width="100%" margin-top="8mm">
          ...
          <fo:table-body>
            <fo:table-row>
              <fo:table-cell font-weight="700" padding-top="2mm">
                <fo:block>Gebäudeadressen:</fo:block>
              </fo:table-cell>
            </fo:table-row>
            <xsl:for-each select="extract:Building">
              <xsl:sort select="extract:Street"/>
              <xsl:sort select="extract:Number"/>
              <xsl:sort select="extract:PostalCode"/>
              <fo:table-row border-width="0pt" border-style="solid">
                <fo:table-cell padding-top="1mm" border-style="solid" border-width="0pt">
                  <fo:block>
                    <xsl:value-of select="extract:PostalAddress/extract:Street"/><xsl:text> </xsl:text><xsl:value-of select="extract:PostalAddress/extract:Number"/><xsl:text>, </xsl:text><xsl:value-of select="extract:PostalAddress/extract:PostalCode"/><xsl:text> </xsl:text><xsl:value-of select="extract:PostalAddress/extract:City"/>
                    <xsl:if test="extract:Egrid">
                        <xsl:text> (EGID: </xsl:text><xsl:value-of select="extract:Egrid"/><xsl:text>) </xsl:text>
                    </xsl:if>
                  </fo:block>
                </fo:table-cell>
              </fo:table-row>
